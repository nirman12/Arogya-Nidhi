...
async function findPatientByUserId(userId) {
  const { data, error } = await supabase
    .from('patients')
    .select(`*, user:users(id,email,name,phone,avatar_url:avatarUrl,role,is_active:isActive,barcode,created_at)`)
    .eq('user_id', userId)
    .maybeSingle();
...
async function findPatientById(id) {
  const { data, error } = await supabase
    .from('patients')
    .select('*, user:users(id,email,name,phone,avatar_url:avatarUrl,barcode)')
    .eq('id', id)
    .maybeSingle();
  if (error) throw error;
  return data;
...
async function updateUserProfile(userId, data) {
  const { data: updated, error } = await supabase
    ...
    .update(data)
    .eq('id', userId)
    .select('id,email,name,phone,avatar_url:avatarUrl,barcode,updated_at:updatedAt')
    .maybeSingle();
  if (error) throw error;
  return updated;
}
